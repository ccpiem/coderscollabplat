...
const Homeredirect = ({theme}) => {
    ...
    let style = `m-2 p-4 rounded-md backdrop-blur-sm 
          hover:box-shadow flex flex-col items-center justify-around xl:text-5xl lg:text-5xl md:text-3xl lg:h-[25vw] xl:h-[25vw] text-dark_blue border-4 border-dark_blue  ${theme ? "bg-gradient-to-t from-[#071fb4]" : "bg-gradient-to-t from-[#ffffff]"} to-[#e2a343]`;
    ...
    return (
        <div className='w-full grid grid-cols-2 grid-template-rows-global md:grid-cols-1 sm:grid-cols-1'>
            ...
            <Link href='https://nitinbhimrajka.github.io/GameJinxx/' onMouseOver={() => sethover8(true)}
            data-aos="zoom-in" data-aos-duration="3000"
                onMouseOut={() => sethover8(false)}>
                <div className={style}> {!ishover8 ? "Games" : <motion.p className='text-2xl md:text-xl'
                    initial={{ opacity: 0, scale: 0.8 }}
                    animate={{ opacity: 1, scale: 1 }}
                    transition={{ duration: 0.3 }}>Play games to enhance your concentration.</motion.p>}</div>
            </Link>
            <Link href='https://ccpiem.github.io/simple-music-player/' onMouseOver={() => sethover7(true)}
            data-aos="zoom-in" data-aos-duration="3000"
                onMouseOut={() => sethover7(false)}>
                <div className={style}> {!ishover7 ? "Music" : <motion.p className='text-2xl md:text-xl'
                    initial={{ opacity: 0, scale: 0.8 }}
                    animate={{ opacity: 1, scale: 1 }}
                    transition={{ duration: 0.3 }}>Listen to soothing music for improving your concentration.</motion.p>}</div>
            </Link>
            <Link href='https://ccpiem.github.io/Code-Editor/' onMouseOver={() => sethover5(true)}
            data-aos="zoom-in" data-aos-duration="3000"
                onMouseOut={() => sethover5(false)}>
                <div className={style}> {!ishover5 ? "CodeEditor" : <motion.p className='text-2xl md:text-xl'
                    initial={{ opacity: 0, scale: 0.8 }}
                    animate={{ opacity: 1, scale: 1 }}
                    transition={{ duration: 0.3 }}>Write and compile your own code here in any preferred programming language.</motion.p>}</div>
            </Link>
            <Link href='https://ccpiem.github.io/webdev/' onMouseOver={() => sethover6(true)}
            data-aos="zoom-in" data-aos-duration="3000"
                onMouseOut={() => sethover6(false)}>
                <div className={style}> {!ishover6 ? "Webdev Editor" : <motion.p className='text-2xl md:text-xl'
                    initial={{ opacity: 0, scale: 0.8 }}
                    animate={{ opacity: 1, scale: 1 }}
                    transition={{ duration: 0.3 }}>Develop amazing Website.</motion.p>}</div>
            </Link>
